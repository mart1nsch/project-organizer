...
def db_to_out(p: database.ProjectDB) -> ProjectOut:
    ...
    return ProjectOut(
        ...
        status=p.status, tags=tags,
        created_at=p.created_at, updated_at=p.updated_at,
    )

def task_to_out(t: database.TaskDB) -> TaskOut:
    return TaskOut(
        id=t.id, project_id=t.project_id, title=t.title,
        notes=t.notes, completed=t.completed, created_at=t.created_at,
    )

def milestone_to_out(m: database.MilestoneDB) -> MilestoneOut:
    return MilestoneOut(
        id=m.id, project_id=m.project_id, title=m.title,
        description=m.description, completed=m.completed, created_at=m.created_at,
    )
...
def list_tasks(project_id: int, db: Session = Depends(database.get_db)):
    return [task_to_out(t) for t in
            db.query(database.TaskDB)
              ...
              .order_by(database.TaskDB.created_at.asc()).all()]
...
def create_task(project_id: int, body: TaskIn, db: Session = Depends(database.get_db)):
    if not db.query(database.ProjectDB).filter(database.ProjectDB.id == project_id).first():
        raise HTTPException(status_code=404, detail="Project not found")
    t = database.TaskDB(
        project_id=project_id, title=body.title.strip(), notes=body.notes or None,
    )
...
def list_milestones(project_id: int, db: Session = Depends(database.get_db)):
    return [milestone_to_out(m) for m in
            db.query(database.MilestoneDB)
              ...
              .order_by(database.MilestoneDB.created_at.asc()).all()]
...
def create_milestone(project_id: int, body: MilestoneIn, db: Session = Depends(database.get_db)):
    if not db.query(database.ProjectDB).filter(database.ProjectDB.id == project_id).first():
        raise HTTPException(status_code=404, detail="Project not found")
    m = database.MilestoneDB(
        project_id=project_id, title=body.title.strip(), description=body.description or None,
    )
